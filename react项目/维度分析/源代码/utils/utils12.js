...
export function geteChartData(data, format) {
	...
	for (let i = 0; i < data.length; i++) {
		arrays.push(new Date(data[i] * 1000).format(format))
	}
	return arrays;
}
/**
...
export function parmToString(parm) {
	...
	for (let k in par['GROUPID']) {
		if (par.modelId && par.modelId[k]) {
			...
			typeStr += par['GROUPID'][k]['group'].toString() + ',';
		}
	}
 ...
 */
export function OKOrCancel(bigModel, dispatch, e, par, latelyParm, getActionOK, getActionCancel) {
	switch (e.currentTarget.innerHTML) {
		case 'OK':
			...
			console.log('====后台传送参数', parm)
			latelyParm = deepCopy(par);
			dispatch(getActionOK(parm));
			break;
		default:
			//清空多选框一切内容
			$(bigModel + ' .itemBox').empty();
			//清除所有勾选选状态
			$(bigModel + ' .insertPosition>ul>li>button').removeClass('button');
			//禁用对应的单选框
			$(bigModel + ' .insertPosition>ul>li>.group').addClass('disableBtn');

			for (let ke in latelyParm.modelId) {
				let itemBox = $(bigModel + ' .itemBox[name = ' + ke + ']'), //对应多选框
					selectBox = itemBox.parent().parent().siblings('button'), //对应勾选框
					groupBox = itemBox.parent().parent().siblings('.select:eq(0)') //对应单选框;

				selectBox.addClass('button'); //为对应勾选框添加勾选状态
				groupBox.removeClass('disableBtn'); //为对应单选框变为可用

				itemBox.append(() => {
					let str = '';
					for (let i = 0, data = latelyParm.modelId[ke]; i < data.length; i++) {
						str += "<li title = " + data[i].text + " pid = " + data[i].pid + ">" + data[i].text + "</li>"
					}
					return str;
				})
			}
			for (let k in latelyParm['GROUPID']) {
				let itemBox = $(bigModel + ' .itemBox[name = ' + k + ']'), //对应多选框
					groupBox = itemBox.parent().parent().siblings('.select:eq(0)'), //对应单选框;
					valGroupBox = groupBox.find('.ant-select-selection-selected-value'),
					groupName = latelyParm['GROUPID'][k]['groupName'];
				valGroupBox.text(groupName);
				valGroupBox.attr('title', groupName);
				dispatch(getActionCancel(groupName, k)); //获取指标下拉框
			}
			break;
	}
}
 ...
 */
export function getOptions(bigModel, dispatch, data, getActionOptions) {
	setTimeout(() => {
		for (let i = 0; i < data['GROUPID'].length; i++) {
			let group = data['GROUPID'][i],
				itemBox = $(bigModel + ' .itemBox[name = ' + i + ']'), //对应多选框
				initData = JSON.parse(sessionStorage.initData); //获取总体筛选数据
			...
			itemBox.append(() => {
				...
				for (let k in data[group]) {
					...
					for (let j = 0; j < initData['modelId'].length; j++) {
						if (k == initData['modelId'][j]['text']) {
							str += "<li title = " + k + " pid = " + initData['modelId'][j].value + ">" + k + "</li>"
							break; //跳出循环
						}
					}
				}
