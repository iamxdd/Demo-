...
class Select extends Handle{
	constructor(opts) {
			...
			this.handlers = {}
			this.config = {
				renderTo:"body", //组件容器css选择器          type:String    
				multiple:false,  //是否多选                   type:Boolean 
				min:null,        //最少选中项                 type:Number
				max:null,        //最多选中项                 type:Number
	...
	createDOM(data){
		...
		$(this.config.renderTo).attr("data-select-uid",uid).attr("placeholder",this.config.placeholder);
	}
	/**
	 *初始化下拉列表大小
	 *
